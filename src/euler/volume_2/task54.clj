...
(defn winning-poker-hands-count
  "Given a sequence of representations of random hands dealt to two players, returns
  number of hands, where 1st player win"
  [coll]
  (let [get-hand (fn [cards]
                   (let [suits {\S :spade \H :heart \C :club \D :diamond}
                         ranks {\2 1 \3 2 \4 3 \5 4 \6 5 \7 6 \8 7 \9 8 \T 9 \J 10 \Q 11
                                \K 12 \A 13}
                         hand (map #(hash-map :suit (get suits (second %))
                                      :rank (get ranks (first %))) cards)
                         sorted-hand (reverse (sort (map :rank hand)))
                         Asorted-hand (reverse (sort (map #(if (= 13 (:rank %))
                                                             0
                                                             (:rank %)) hand)))
                         flush? (= 1 (count (group-by :suit hand)))
                         straight? (or (= 4 (- (first sorted-hand) (last sorted-hand)))
                                     (= 4 (- (first Asorted-hand) (last Asorted-hand))))
                         group-value #(+ (* (last %) 20) (first %))
                         groups (sort #(compare (group-value %1) (group-value %2))
                                  (filter #(> (last %) 1)
                                    (map #(vector (key %) (count (val %)))
                                      (group-by identity
                                        sorted-hand))))
                         extract-group (let [group (first (first groups))]
                                         (concat [group]
                                           (remove #(= % group) sorted-hand)))
                         extract-2groups (let [group-keys (map first groups)
                                               group (last group-keys)
                                               group2 (first group-keys)]
                                           (concat [group group2]
                                             (remove #(or (= % group) (= % group2))
                                               sorted-hand)))]
                     (cond (and flush? straight? (empty? groups)) (conj sorted-hand 8)
                       (and (not-empty groups) (= 4 (last (first groups))))
                       (conj extract-group 7)
                       (and (= 2 (count groups)) (= 3 (last (last groups))))
                       (conj extract-2groups 6)
                       flush? (conj sorted-hand 5)
                       (and straight? (empty? groups)) (conj sorted-hand 4)
                       (and (not-empty groups) (= 3 (last (first groups))))
                       (conj extract-group 3)
                       (= 2 (count groups)) (conj extract-2groups 2)
                       (not-empty groups) (conj extract-group 1)
                       :else (conj sorted-hand 0))))
        hands-comp (fn hands-comp [x y]
                     (let [res (compare (first x) (first y))]
                       (if (= 0 res)
                         (hands-comp (rest x) (rest y))
                         res)))]
    (get (frequencies (map #(hands-comp (get-hand (first %)) (get-hand (last %)))
                        (map #(partition 5 (clojure.string/split % #" ")) coll))) 1)))

(def arr1 ["8C TS KC 9H 4S 7D 2S 5D 3S AC"
